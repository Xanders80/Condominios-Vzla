...
(function ($) {
    $.loadModal = function (options) {
        ...
        idBody: 'jquery-load-modal-js-body',                    // the id of the modal-body (the dialog content)
            ...
            title: window.document.title || {{ trans('Dialog') }}, // the title of the dialog
        ...
        options.ajax.success.unshift(function (data, status, xhr) { // unshift puts this as the first success method
            ...
            const div = $([
                ...
                '      <div class="modal-content bg-gradient-warning-dark">',
                '        <div class="modal-header border-bottom ' + options.bgClass + '" style="cursor: move;" title="{{ trans('Click on [X] Close Window') }}">',
                ...
                '           <button class="btn btn-sm btn-danger pull-up" data-bs-dismiss="modal"><i class="mdi mdi-cancel "></i> {{ trans('Close') }} </button>',
                '           <button type="button" data-for="" class="pull-up pull-right btn btn-info btn-sm submit-data btn-' + options.bgClass + ' submit-' + submitClass + '" style="' + options.btnStyle + '">' +
                '               <i class="mdi mdi-refresh mdi-spin " style="display:none;"></i><i class="mdi mdi-content-save "></i> ' + options.title +
                '           </button>',
                '        </div>',
                '      </div>',
                '    </div>',
    ...
    $.showLoading = function () {
        $.blockUI({
            message: '<div class="loading"><i class="mdi mdi-camera-timer mdi-spin mdi-24px"></i> {{ trans('Loading...')}}</div>',
            css: {
                'border': 'none',
                'padding': '15px',
                'backgroundColor': '#000',
