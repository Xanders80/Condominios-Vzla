...
<div class="row text-center">
    <div class="col-md-12">
        ...
        <p>{{ trans('Are you sure you want to delete this motion?') }}</p>
        <p><strong>{{ __('Title') }}:</strong> {{ $data->title }}</p>
        <p><strong>{{ __('Session Date') }}:</strong> {{ $data->assemblySession->session_date }}</p>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('.modal-title').html('<i class="mdi mdi-delete mdi-24px text-danger"></i> - {{ trans('Delete Motion') }}');
