        ...
        <div class="row"> {{-- - Nombre & Rif - --}}
            <div class='col-6'>
                <x-input-text name="name" label="{{ trans('Name') }}" dataUser="{{ $data->name }}"
                    plHolder="{{ trans('Type here...') }}" icon="mdi mdi-map-marker-minus " id="name" isRequired=true
                    autofocus />
            </div>
            <div class='col-6'>
                <x-input-select icon="mdi mdi-home-modern" id="tower_sector_id" label="{{ trans('Tower Sector') }}"
                    isRequired=true>
                    {!! html()->select('tower_sector_id', $towerSector, $data->tower_sector_id)->placeholder('Choose here')->class('form-control select2')->id('tower_sector_id') !!}
                </x-input-select>
            </div>
        </div>
    </div>
