...
<div class="panel shadow-sm" style="border-radius: 10px;">
    <div class="panel-body">
        <div class="row"> {{-- - Piso Calle - --}}
            <div class='col-6'>
                <x-input-text name="name" label="{{ trans('Name') }}" plHolder="{{ trans('Type here...') }}"
                    icon="mdi mdi-map-marker-minus " id="name" isRequired=true autofocus />
            </div>
            <div class='col-6'>
                <x-input-select icon="mdi mdi-home-modern" id="tower_sector_id" label="{{ trans('Tower Sector') }}"
                    isRequired=true>
                    {!! html()->select('tower_sector_id', $towerSector)->placeholder('Choose here')->class('form-control select2')->id('tower_sector_id')->required() !!}
                </x-input-select>
            </div>
        </div>
    </div>
