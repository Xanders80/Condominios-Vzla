        ...
        <div class="row">
            <div class='form-group col-4'> {{-- - Nombre - --}}
                <x-input-text name="name" label="{{ trans('Name') }}" plHolder="{{ trans('Type here...') }}"
                    icon="mdi mdi-home-modern " id="name" isRequired=true autofocus />
            </div>
            <div class='form-group col-4'> {{-- - Tipo de Unidad - --}}
                <x-input-select id="unit_type_id" label="{{ trans('Unit Type') }}" isRequired=true>
                    {!! html()->select('unit_type_id', $unitTypes)->placeholder(trans('Choose Menu'))->class('form-control select2')->id('unit_type_id') !!}
                </x-input-select>
            </div>
            <div class='form-group col-4 mt-4'> {{-- - Estado de Unidad - --}}
                <x-input-checkbox id="md_checkbox" name="status" label="{{ trans('Inhabited') }}" class="checkbox"
                    valor="1" />
            </div>
        </div>
        <div class="row">
            <div class='form-group col-4'> {{-- - Propietario de la Unidad - --}}
                <x-input-select id="dweller_id" label="{{ trans('Unit Owner') }}" isRequired=true>
                    {!! html()->select('dweller_id', $dweller)->placeholder(trans('Choose Menu'))->class('form-control select2')->id('dweller_id') !!}
                </x-input-select>
            </div>
            <div class='form-group col-4'> {{-- - Piso o Calle - --}}
                <x-input-select id="tower_sector_id" label="{{ trans('Tower Sector') }}" isRequired=true>
                    {!! html()->select('tower_sector_id', $towerSector)->placeholder(trans('Choose Menu'))->class('form-control select2')->id('tower_sector_id') !!}
                </x-input-select>
            </div>
            <div class='form-group col-4'> {{-- - Piso o Calle - --}}
                <x-input-select id="floor_street_id" label="{{ trans('Floor Street') }}" isRequired=true>
                    {!! html()->select('floor_street_id', [])->placeholder(trans('Choose Menu'))->class('form-control select2')->id('floor_street_id') !!}
                </x-input-select>
            </div>
        </div>
    </div>
