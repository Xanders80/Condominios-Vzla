...
<div class="panel shadow-sm" style="border-radius: 10px;">
    <div class="panel-body">
        <div class="row"> {{-- - Nombre & Rif - --}}
            <div class='form-group col-6'>
                <x-input-text name="name" label="{{ trans('Name') }}" plHolder="{{ trans('Type here...') }}"
                    icon="mdi mdi-city" id="name" isRequired=true autofocus />
            </div>
            <div class='form-group col-6'>
                <x-input-text name="rif" label="{{ trans('Rif') }}" plHolder="{{ trans('Type here...') }}"
                    icon="mdi mdi-account-card-details " id="rif" isRequired=true autofocus />
            </div>
        </div>

        <div class="row"> {{-- - Encargado - --}}
            <div class='form-group col-6'>
                <x-input-text name="name_incharge" label="{{ trans('Name In Charge') }}" dataUser="{{ $nameInCharge }}"
                    plHolder="{{ trans('Type here...') }}" icon="mdi mdi-account-settings-variant " id="name_incharge"
                    isRequired=true autofocus />
            </div>
            <div class='form-group col-6'>
                <x-input-text name="jobs_incharge" label="{{ trans('Jobs In Charge') }}"
                    plHolder="{{ trans('Type here...') }}" icon="mdi mdi-tie " id="jobs_incharge" isRequired=true
                    autofocus />
            </div>
        </div>
        ...
        <div class="row"> {{-- - Correo & Teléfono - --}}
            <div class='form-group col-6'>
                <x-input-text name="email" label="{{ trans('Email') }}" plHolder="{{ trans('Type here...') }}"
                    icon="mdi mdi-email " id="email" isRequired=true autofocus />
            </div>
            <div class='form-group col-6'>
                <x-input-text name="phone" label="{{ trans('Number') }}" icon="mdi mdi-phone-classic "
                    plHolder="{{ trans('Type Phone') }}" id="phone" isRequired=true autofocus />
            </div>
        </div>
        ...
        <div class="address-group mt-2"
            ...
            <div class="row"> {{-- - Dirección - --}}
                <div class='form-group col-6'>
                    <x-input-text name="address_line" label="{{ trans('Address: Zone / Sector / Urbanism / Neighborhood') }}"
                        plHolder="{{ trans('Type here... and press Enter to Search') }}" icon="mdi mdi-home-map-marker " id="address_line"
                        isRequired=true autofocus />
                </div>

                <div class="col-6">
                    <x-input-select icon="mdi mdi-map-marker-radius" id="postal_code_address" label="{{ trans('Full Address') }}" isRequired=true>
                        {!! html()->select('postal_code_address', [])->class('form-control select2')->id('postal_code_address')->placeholder(trans('Choose Menu'))->required() !!}
                    </x-input-select>
                </div>
            </div>
        </div>
        ...
        <div class="row"> {{-- - Porcentajes Cobro y Actividad - --}}
            <div class="row col-9">
                @foreach ([['label' => 'Reserve Found', 'name' => 'reserve_found', 'icons' => 'mdi mdi-cash-multiple'], ['label' => 'Rate Percentage', 'name' => 'rate_percentage', 'icons' => 'mdi mdi-percent'], ['label' => 'Billing Date', 'name' => 'billing_date', 'icons' => 'mdi mdi-calendar-today']] as $input)
            ...
            <div class='form-group col-sm-6 col-md-3 mt-4'>
                <x-input-checkbox id="md_checkbox" name="active" label="{{ trans('Active') }}" class="checkbox"
                    valor="1" />
            </div>
        </div>
        ...
        <div class='form-group'> {{-- - Logo - --}}
            <x-input-text name="logo" label="{{ trans('Logo') }}" plHolder="{{ trans('Type here...') }}"
                icon="mdi mdi-image " id="logo" isRequired=true autofocus />
        </div>
    </div>
</div>
<div class="row">
