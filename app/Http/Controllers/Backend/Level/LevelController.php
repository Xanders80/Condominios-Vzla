...
/**
 * Clase que maneja las operaciones de los niveles.
 */
class LevelController extends Controller
{
    /**
     * Muestra la vista principal de los niveles.
     *
    ...
    /**
     * Muestra el formulario de edición del recurso especificado.
     *
     * @param int $id identificador del recurso
     *
     * @return \Illuminate\View\View
    ...
    /**
     * Formatea el acceso a niveles como una cadena de badges.
     *
     * @param array $access
     *
     * @return string
     */
    private function formatAccess($access)
    {
        $badges = '';
        foreach (collect(config('master.app.level')) as $level) {
            if (in_array($level, $access)) {
                $badgeClass = $access[$level] ? 'badge-success' : 'badge-gray disabled';
                $badges .= '<span class="badge rounded-pill ' . $badgeClass . '">' . trans($level) . '</span> ';
            }
        }
    ...
    /**
     * Maneja el almacenamiento o actualización de un nivel en la base de datos.
     *
     * @param string   $action 'store' o 'update'
     * @param int|null $id     el ID del nivel a actualizar (opcional)
     *
     * @return \Illuminate\Http\JsonResponse
     */
    private function handleStoreOrUpdate(Request $request, string $action, ?int $id = null)
    {
        try {
            $response = [];
