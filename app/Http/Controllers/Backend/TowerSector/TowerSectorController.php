...
namespace App\Http\Controllers\Backend\TowerSector;
...
class TowerSectorController extends Controller
{
    /**
    ...
    public function data(Request $request): object
    {
        try {
            ...
            Log::error('Error fn(TowerSectorController) data', [
    ...
    private function handleStoreOrUpdate(Request $request, string $action, ?string $id = null)
    {
        try {
            ...
            $response['status'] = false;
            $response['message'] = trans(config('constants.MESSAGES.DATA_ERROR')) . ' Tower Sector ' . $action . ': ' . $e->getMessage();
            $response['status_code'] = config('constants.STATUS_CODES.INTERNAL_SERVER_ERROR');
            Log::error('Error fn(TowerSectorController) handleStoreOrUpdate', [
                'detail' => trans(config('constants.MESSAGES.DATA_ERROR')),
                'error' => $e->getMessage(),
    ...
    private function prepareViewData(string $action, $dataModel): array
    {
        ...
        switch ($action) {
            case 'create':
            case 'edit':
                $viewData['condominiums'] = Condominiums::pluck('name', 'id');
                break;
            case 'delete':
                break;
            default:
