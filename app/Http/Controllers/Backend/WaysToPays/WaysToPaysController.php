...
namespace App\Http\Controllers\Backend\WaysToPays;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class WaysToPaysController extends Controller
{
    /**
     * Muestra la vista del índice de métodos de pago.
     *
    ...
    /**
     * Muestra la vista para crear un nuevo método de pago.
     *
     * @return \Illuminate\View\View
     */
    ...
    /**
     * Muestra la vista de edición de un método de pago existente.
     *
     * @param int $id
     *
     * @return \Illuminate\View\View
    ...
    public function data(Request $request): object
    {
        try {
            ...
            Log::error('Error fn(WaysToPaysController) data', [
    ...
    /**
     * Elimina un método de pago específico de la base de datos.
     *
     * @param int $id
     *
     * @return \Illuminate\Http\JsonResponse
    ...
    public function destroy($id)
    {
        try {
            ...
            Log::error('Error fn(WaysToPaysController) destroy', [
    ...
    private function handleViewAction(string $action, ?string $id = null)
    {
        try {
            ...
            Log::error('Error fn(WaysToPaysController) handleViewAction', [
                'detail' => trans(config('constants.MESSAGES.DATA_ERROR')) . trans(config('constants.MESSAGES.ERROR_DISPLAYING_MODEL')),
                'error' => $e->getMessage(),
    ...
    /**
     * Maneja el almacenamiento o actualización de un método de pago.
     *
     * @return \Illuminate\Http\JsonResponse
     */
    private function handleStoreOrUpdate(Request $request, string $action, ?string $id = null)
    {
        try {
            ...
            $response['status_code'] = config('constants.STATUS_CODES.INTERNAL_SERVER_ERROR');
            Log::error('Error fn(WaysToPaysController) handleStoreUpdate', [
                'detail' => trans(config('constants.MESSAGES.DATA_ERROR')),
                'error' => $e->getMessage(),
                'data' => 'Action: ' . $action . ', ID: ' . $id,
            ]);
