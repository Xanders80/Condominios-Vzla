...
class UpdateUserRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        // Obtener el ID del usuario de la ruta o del modelo inyectado
        // Asumiendo que el parámetro de ruta se llama 'user' o se pasa un ID
        $id = $this->route('user') ? $this->route('user') : $this->input('id');

        // Si el usuario viene como objeto (Route Model Binding), obtenemos su ID
        if ($id instanceof User) {
            $id = $id->id;
        }

        // La contraseña es opcional al actualizar
        $isRequiredPassword = $this->filled('password') ? 'required' : 'nullable';
        ...
        return [
            ...
            'email' => [
                ...
                User::EMAIL_MAX_RULE,
                // Ignorar el email del usuario actual. $id puede ser nulo si no se pasa correctamente, cuidado.
                'unique:' . User::class . ',email,' . $id . ',id,deleted_at,NULL',
            ],
            'password' => [
                $isRequiredPassword,
                'confirmed',
